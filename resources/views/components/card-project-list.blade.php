...
@php	 
		...
		if($days <= 0 ){
			$daysleft = 	__('meta.projects.application_closed');
			$class = "text-red-400";
			$apply_class = "hidden";
		} else {
			$daysleft = $days . trans_choice('meta.projects.left_to_apply', $days);
			$class="text-blue-800";
			$apply_class = "";
		}
...
<x-blue-card class="{{$closed}}">
		<div {{ $attributes->merge(['class' => 'card-project_header flex justify-between items-center flex-wrap  gap-y-2']) }}>
			<div class="card-project_header-left flex items-center">
				...
				<div class="card-header-company_info ">
					<h3 class="project-title text-base font-bold">
						{{ $project->id .	" - " . $project->project_title }}
					</h3>
					<div class="card-company_meta text-sm">
						{{__('meta.projects.company')}} 
						<a href="/projects/search/?project_company={{ htmlspecialchars($project->project_company)  }}" class="text-blue-600 hover:text-blue-800">		
							{{ urldecode($project->project_company) }}
						</a>, 		
						{{__('meta.projects.category')}}   
						<a href="/projects/search/?project_category={{ $project->project_category }}" class="text-blue-600 hover:text-blue-800">
							{{ __('project_categories.'.$category_str) }}
						</a>
					</div>
					@unless ($skills === "")
						<div class="card-company_meta text-sm">
		...
		<div class="flex justify-between items-center	 mt-4 text-blue-500 mr-2 flex-col md:flex-col   ">
			<div class="flex justify-between flex-row gap-4 text-xs md:text-sm flex-wrap md:flex-nowrap gap-y-2 w-full">
				<div>
				...
				<div>
					<span class="{{$apply_class}}">
						<x-btn-darkblue class="hover:bg-blue-400" href="javascript:void(0)">
							Apply
						</x-btn-darkblue>
					</span>
				</div>
